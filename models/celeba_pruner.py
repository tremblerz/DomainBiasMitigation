...
from models.PrunerModel import PruningNetwork
import utils
...
class CelebaPruner(CelebaModel):
    def __init__(self, opt):
        super(CelebaPruner, self).__init__(opt)
    ...
    def set_network(self, opt):
        ...
        rand_img = torch.rand(1, 3, 128, 128).to(self.device)
        num_channels = self.base_network(rand_img).shape[1]
        hparams = {"pruning_ratio": 0.6, "pruning_style": "network", "logits": num_channels,
                   "split_layer": split_layer}
        self.pruner_network = PruningNetwork(hparams).to(self.device)
    ...
    def set_optimizer(self, opt):
        ...
        self.domain_optimizer = optimizer_setting['optimizer']( 
                                  ...
                                  lr=optimizer_setting['lr'],
                                  weight_decay=optimizer_setting['weight_decay'])
        self.pruner_optimizer = optimizer_setting['optimizer'](
                                  params=filter(lambda p: p.requires_grad, self.pruner_network.parameters()),
                                  lr=optimizer_setting['lr'],
                                  weight_decay=optimizer_setting['weight_decay'])
    ...
    def load_state_dict(self, state_dict):
        ...
        self.pruner_network.load_state_dict(state_dict['pruner_network'])
    ...
    def _train(self, loader):
        ...
        for i, (images, targets) in enumerate(loader):
            ...
            features_detached = Variable(features.detach().clone(), requires_grad=True)
            features_pruned, indices = self.pruner_network(features_detached)

            class_input = Variable(features_pruned.detach().clone(), requires_grad=True)
            class_outputs = self.class_network(class_input)

            domain_input = Variable(features_pruned.detach().clone(), requires_grad=True)
            domain_outputs = self.domain_network(domain_input)
            ...
            features_pruned.backward(class_input.grad, retain_graph=True)
            features.backward(features_detached.grad)
            self.base_optimizer.step()

            features_pruned.backward(- domain_input.grad)
    ...
    def _test(self, loader):
        ...
        with torch.no_grad():
            for i, (images, targets) in enumerate(loader):
                images, targets = images.to(self.device), targets.to(self.device)
                features = self.base_network(images)
                features_pruned, indices = self.pruner_network(features)
                class_outputs = self.class_network(features_pruned)
                domain_outputs = self.domain_network(features_pruned)
